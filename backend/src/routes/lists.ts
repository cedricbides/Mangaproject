...
// Public lists for a given user
router.get('/user/:userId', async (req: Request, res: Response) => {
  try {
    const lists = await MangaList.find({ userId: req.params.userId, isPublic: true }).sort({ createdAt: -1 })
    res.json(lists)
  } catch (err: any) { res.status(500).json({ error: err.message }) }
})

// All lists owned by the logged-in user (including private)
...
// Single list: public when isPublic is true; otherwise owner-only.
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const list = await MangaList.findById(req.params.id)
    if (!list) return res.status(404).json({ error: 'List not found' })
    const userId = (req.user as any)?.id
    if (!list.isPublic && list.userId !== userId) return res.status(403).json({ error: 'Private list' })
    res.json(list)
  } catch (err: any) { res.status(500).json({ error: err.message }) }
})
