...
// ── Public: heartbeat (called from frontend every 30s) ────────────────────────
...
// ── Admin: get current active visitors ────────────────────────────────────────
...
// ── Admin: visitor history (last 24h buckets) ─────────────────────────────────
router.get('/history', requireAdmin, async (_req, res) => {
  try {
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000)
    const sessions = await VisitorSession.find({ createdAt: { $gte: oneDayAgo } })
